 ...
 * 兼容 Surge 标准 API，使用 module.exports
 */
...
module.exports = async function (ctx) {
  ...
  // 读取客户端名称：优先使用模块参数 $argument，否则从环境变量读取，默认 Telegram
  let client = ($argument || process.env.CLIENT || "Telegram").trim();
  const key = client.toLowerCase();
  const scheme = SCHEME[key] || "tg"; // 未匹配则默认 tg
