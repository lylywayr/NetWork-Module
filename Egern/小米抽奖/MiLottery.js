...
function Env(s) {
    ...
    this.isEgern = () => typeof Egern !== "undefined";
    this.log = (...args) => console.log(args.join("\n"));
    this.msg = (title = this.name, subtitle = "", body = "") => {
        if (this.isSurge() || this.isLoon() || this.isEgern()) $notification.post(title, subtitle, body);
        else if (this.isQuanX()) $notify(title, subtitle, body);
        console.log(["", `====📣${title}====`, subtitle, body].filter(Boolean).join("\n"));
    };
    this.getdata = (key) => {
        if (this.isSurge() || this.isLoon() || this.isEgern()) return $persistentStore.read(key);
        if (this.isQuanX()) return $prefs.valueForKey(key);
        return null;
    };
    this.setdata = (value, key) => {
        if (this.isSurge() || this.isLoon() || this.isEgern()) return $persistentStore.write(value, key);
    ...
    this.send = (request, method, callback) => {
        if (this.isSurge() || this.isLoon() || this.isEgern()) {
